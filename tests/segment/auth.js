...
it('Should generate proper headers using _getPrivateHeaders', function () {
  ...
  expect(result).to.have.property('Content-Type', 'application/x-www-form-urlencoded');
...
it('Should test verify valid', function () {
  ...
  stub = sinon.stub(client, '_getIpnHMAC').withArgs(merchantSecret, payload).returns(hmac);
...
it('Should test verify invalid', function () {
  ...
  stub = sinon.stub(client, '_getIpnHMAC').withArgs(merchantSecret, payload).returns('xxxxxxxxxxx');
